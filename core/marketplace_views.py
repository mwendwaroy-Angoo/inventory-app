...
def shop_home(request):
    ...
    business_ids = (
        Item.objects.filter(selling_price__isnull=False)
...
def storefront(request, business_id):
    ...
    if store_filter:
        available_items = [
            i for i in available_items if str(i.store_id) == store_filter
        ]
    if search:
        available_items = [
            i for i in available_items if search.lower() in i.description.lower()
        ]

    # Delivery tiers as JSON for frontend
    delivery_tiers_data = list(
        business.delivery_tiers.values(
...
def place_order(request, business_id):
    ...
    for entry in cart:
        ...
        if qty > available:
            adjustments.append(
                f"{item.description}: adjusted to {available} (only {available} available)"
            )
            qty = available

        if not item.selling_price:
            return JsonResponse(
                {"error": f"{item.description} has no price set"}, status=400
            )
    ...
    if min_order > 0 and subtotal < min_order:
        return JsonResponse(
            {"error": f"Minimum order amount is KES {min_order:,.0f}"}, status=400
        )
...
def pay_order(request, order_number):
    ...
    paid_amount = sum(p.amount for p in order.payments.filter(status="completed"))
...
def update_order_status(request, order_id):
    ...
    if (
        ...
        and not Transaction.objects.filter(invoice_no=order.order_number).exists()
    ):
...
def assign_rider(request, order_id):
    ...
    # Notify the rider and business owner about the assignment
    try:
        from .notifications import (
            notify_rider_delivery_assigned,
            notify_business_rider_assigned,
        )

        notify_rider_delivery_assigned(rider, order)
        notify_business_rider_assigned(order, rider)
    except Exception:
        pass  # Don't block rider assignment if notifications fail
...
def supplier_list(request):
    ...
    return render(
        request,
        "marketplace/supplier_list.html",
        {
            "links": links,
        },
    )
        ...
        if supplier_id:
            ...
            if supplier == my_business:
                django_messages.error(
                    request, "You can't add your own business as a supplier."
                )
            elif SupplierRelationship.objects.filter(
                business=my_business, supplier=supplier
            ).exists():
                django_messages.warning(
                    request, f"{supplier.name} is already in your supplier list."
                )
            else:
                ...
                django_messages.success(
                    request, f"{supplier.name} added as a supplier."
                )
        return redirect("supplier_list")

    # Show businesses on the platform (excluding own)
    query = request.GET.get("q", "").strip()
    businesses = Business.objects.exclude(id=my_business.id).select_related(
...
def edit_supplier(request, link_id):
    ...
    link = get_object_or_404(
        SupplierRelationship, id=link_id, business=profile.business
    )
