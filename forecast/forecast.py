...
def get_revenue_df(
    db_path="db.sqlite3", business_id=None, start_date=None, end_date=None
):
...
def fit_ets_forecast(series, steps=30, cadence="daily"):
    ...
    This function is resilient: if `statsmodels` is not available or model
    fitting fails, it falls back to a naive forecast (repeat-last-value or
    zeros for empty series). This prevents the whole request/task from
    failing when the package isn't installed or when the model cannot be fit.
    """
    ...
    # Empty input -> return zeros for the requested horizon
    if series.empty:
        freq_map = {"daily": "D", "weekly": "W-MON", "monthly": "M"}
        freq = freq_map.get(cadence, "D")
        idx = pd.date_range(start=pd.Timestamp.today(), periods=steps, freq=freq)
    ...
    # Try statsmodels model when available
    if _HAS_STATS:
        try:
            ...
            # Fall back to naive forecast below
            pass

    # Naive fallback: repeat last observed value (or 0 if none)
    last_vals = series.dropna()
    last = float(last_vals.iloc[-1]) if len(last_vals) else 0.0
    # Attempt to preserve frequency; fall back to cadence default
    freq = (
        series.index.freq
        or pd.infer_freq(series.index)
        or {"daily": "D", "weekly": "W-MON", "monthly": "M"}.get(cadence, "D")
    )
    start = series.index[-1]
    try:
        first = start + pd.tseries.frequencies.to_offset(freq)
    except Exception:
        first = pd.Timestamp.today()
        freq = {"daily": "D", "weekly": "W-MON", "monthly": "M"}.get(cadence, "D")

    idx = pd.date_range(start=first, periods=steps, freq=freq)
...
def main():
    ...
    p.add_argument(
        ...
        help="Aggregation cadence",
    )
    p.add_argument(
        "--horizon", type=int, default=30, help="Forecast horizon (number of periods)"
    )
    p.add_argument(
        "--business", type=int, default=None, help="Business id to filter (optional)"
    )
    p.add_argument(
        "--output-dir", default="forecast/output", help="Directory to write outputs"
    )
    args = p.parse_args()
